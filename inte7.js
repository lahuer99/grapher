...
// $('vennnos').hide();
  ...
  for(var i=0;i<$('#vennnosinp').val();i++){
    $('#basecontainer').append('<div><input style="width:30%" type="text" id="base'+i+'"/></div>');
  }
    $('#basecontainer').show();
...
if(x<nosub){
  $('#subsetcontainer').append('<div id="maindiv"><div><input type="text" id="basenext'+wnosubs+'"/></div> belongs to <div id="newselect"></div></div><br>');
  wnosubs+=1;
}
